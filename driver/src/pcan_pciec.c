...
/**
 * This function is called cyclic from a timer
 * @param ptr The associated card structure, must be casted to PCAN_PCIEC_CARD *
 */
static void pciec_activity_scanner(unsigned long ptr)
{
  PCAN_PCIEC_CARD *card = (PCAN_PCIEC_CARD *)ptr;
  struct pcandev *dev;
  int i;
  u8 tmp;
...
static void pciec_start_activity_scanner(PCAN_PCIEC_CARD *card)
{
  ...
  card->activity_timer.function = pciec_activity_scanner;
